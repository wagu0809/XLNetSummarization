...
def cal_rouge(args):
    ...
    tensorboard_writer = SummaryWriter(args.model_path, comment="Unmt")
    rouges = _report_rouge(gold_path, can_path)
    logger.info('Rouges at step %d \n%s' % (step, rouge_results_to_str(rouges)))
    if tensorboard_writer is not None:
        tensorboard_writer.add_scalar('test/rouge1-F', rouges['rouge_1_f_score'], step)
        tensorboard_writer.add_scalar('test/rouge2-F', rouges['rouge_2_f_score'], step)
        tensorboard_writer.add_scalar('test/rougeL-F', rouges['rouge_l_f_score'], step)

def _report_rouge(gold_path, can_path):
    logger.info("Calculating Rouge")
    results_dict = test_rouge(args.temp_dir, can_path, gold_path)
    return results_dict

if __name__ == "__main__":
    init_logger('./logs/test_rouge.log')
    ...
    parser.add_argument('-c', type=str, default="./results/cnndm.10000.candidate",
                        help='candidate file')
    parser.add_argument('-r', type=str, default="./results/cnndm.10000.gold",
                        help='reference file')
    parser.add_argument('-p', type=int, default=1,
                        help='number of processes')
    parser.add_argument('-log', type=str, default='./logs/test_rouge.log')
